...
//          spinMultiplicity{std::move(spinMultiplicity)} {}
  ...
  struct atom_node {
    ...
    boost::optional<std::string> x2{boost::none}, y2{boost::none};
    ...
    std::string str() const {
      ...
      put_unless_none{}(ss, "x2", x2);
      put_unless_none{}(ss, "y2", y2);
      ss << "/>";
      return ss.str();
    }
  };
  ...
  WHEN("//molecule/atomArray/atom/@x2") {
    AND_WHEN("is missing") {
      atom_node a;
      a.x3 = boost::none;
      a.y3 = boost::none;
      ...
      REQUIRE_THROWS_WITH(
          CMLBlockToMol(cml),
          Matches(".+/atom[^/]* does not have both of x2 and y2 attributes"));
    }
  }

  WHEN("//molecule/atomArray/atom/@x2 and /@x3") {
    AND_WHEN("is missing") {
      atom_node a;
      a.x2 = "0.0";
      const auto cml = cml_root(molecule_node{}.str(cml_atomArray(a.str())));
      REQUIRE_THROWS_WITH(
          CMLBlockToMol(cml),
          Matches(".+/atom[^/]* has both of 3D.* and 2D.* attributes"));
    }
  }

  WHEN("//molecule/bondArray") {
    AND_WHEN("has no bond") {
