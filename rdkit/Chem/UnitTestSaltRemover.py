...
class TestCase(unittest.TestCase):
  ...
  def test_withUseChirality(self):
    chiralRemover = SaltRemover(defnData=r"OC(=O)\C=C/C(O)=O\nOC(=O)/C=C/C(O)=O", defnFormat=InputFormat.SMILES, useChirality=True)
    remover = SaltRemover(defnData=r"OC(=O)\C=C/C(O)=O\nOC(=O)/C=C/C(O)=O", defnFormat=InputFormat.SMILES, useChirality=False)
    
    maleaic_acid_smiles = Chem.MolToSmiles(Chem.MolFromSmiles(r'OC(=O)\C=C/C(O)=O'))
