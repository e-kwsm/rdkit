...
class TestCase(unittest.TestCase):
  ...
  def test3(self):
    data = [
      ('[O-][N+](C1=C([N+]([O-])=O)C=C([N+]([O-])=O)C=C1)=O', 3),
      ('[O-][N+](C1=CC=CC=C1)=O', 1),
  ...
  def test4(self):
    data = [
      ('[O-][N+](C1=C([N+]([O-])=O)C=C([N+]([O-])=O)C=C1)=O', 1),
      ('[O-][N+](C1=CC=CC=C1)=O', 1),
      ('[N+]([O-])(=O)C1OC(=CC=1)/C=N/NC(N)=O', 0),
      ('O=C(O)C(=O)O', 0),
    ]
  ...
  def test5(self):
    data = [
      ('C1(N=[N+]=[N-])=C(C(=C(C(=C1F)F)F)F)F', 1),
    ]
  ...
  def test13(self):
    data = [
      ('CNC1(CCCCC1=O)C2(=CC=CC=C2Cl)', 1),  # ketamine
  ...
  def test14(self):
    data = [
      ('C12(=C(N=CC=C1[C@@H](O)C3(N4CC(C(C3)CC4)([H])C=C)[H])C=CC(=C2)OC)', 0),  # quinine
  ...
  def test17(self):
    data = [
      ('O=C(C=CC1=CC=CC=C1)C=CC2=CC=CC=C2', 0),  # a,b-unsat. dienone
  ...
  def test18(self):
    data = [
      ('C1=CC(=CC=C1C(N[C@@H](CCC(O)=O)C(O)=O)=O)N(CC2=NC3C(=NC(=NC=3N=C2)N)N)C',
       2),  # methotrexate
  ...
  def test19(self):
    data = [
      ('CC(C)(C)NCC(O)C1=CC(O)=CC(O)=C1', 1),  # terbulatine
