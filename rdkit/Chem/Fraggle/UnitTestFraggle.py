...
class TestCase(unittest.TestCase):
  ...
  def test_generate_fraggle_fragmentation(self):
    ...
    frags = FraggleSim.generate_fraggle_fragmentation(mol)
    expected = [
      '*CCC.*c1ccccc1*', '*Cc1cccc2ccccc12', '*c1cccc(CCC)c1*', '*c1cccc2ccccc12', '*c1ccccc1*'
    ]
    expected = [_of(s) for s in expected]
    for smi in frags:
      self.assertIn(_of(smi), expected)
  ...
  def test_select_fragments_acyclic(self):
    ...
    self._testSelectFragment('*CC.*O*.*c1ccccc1', FraggleSim.FTYPE_ACYCLIC, 9, '*c1ccccc1')
    self._testSelectFragment('*CC.*OCC*.*c1ccccc1', FraggleSim.FTYPE_ACYCLIC, 11, '*c1ccccc1')
    ...
    self._testSelectFragment('*CC.*O*.*c1ccccc1', FraggleSim.FTYPE_ACYCLIC, 9, '*c1ccccc1')
    self._testSelectFragment('*CCC.*O*.*c1ccccc1', FraggleSim.FTYPE_ACYCLIC, 10, '*CCC.*c1ccccc1')
    # Only small fragments
    self._testSelectFragment('*CC.*CC.*CC', FraggleSim.FTYPE_ACYCLIC, 6, None)
    self._testSelectFragment('*CCC.*CCC.*CCC', FraggleSim.FTYPE_ACYCLIC, 6, '*CCC.*CCC.*CCC')

  def test_select_fragments_cyclic(self):
    self._testSelectFragment('*c1ccccc1*.*cccc*', FraggleSim.FTYPE_CYCLIC, 10, '*c1ccccc1*')
    self._testSelectFragment('*c1ccccc1*.*C.*C', FraggleSim.FTYPE_CYCLIC, 8, None)
    # Fragment too small
    self._testSelectFragment('*c1ccccc1*.*c(CCCCCCCCCC)ccc*', FraggleSim.FTYPE_CYCLIC, 20, None)
  ...
  def test_select_fragments_cyclic_acyclic(self):
    ...
    self._testSelectFragment('*CCC.*C.*c1ccccc1*', FraggleSim.FTYPE_CYCLIC_ACYCLIC, 10,
                             '*CCC.*c1ccccc1*')
    self._testSelectFragment('*CCC.*CCCCCCCCCCC*.*c1ccccc1*', FraggleSim.FTYPE_CYCLIC_ACYCLIC, 25,
                             None)
