...
class TestCase(unittest.TestCase):
  ...
  def testPCA(self):
    ...
    tVect = numpy.array([[-0.64200458, -0.66321742,
                          0.38467229], [0.34166917, 0.20166619, 0.91792861],
    ...
    for i in range(eVects.shape[0]):
      assert (max((abs(eVects[i] - tVect[i])).ravel()) < FLOAT_TOL or max(
        (abs(eVects[i] + tVect[i])).ravel()) < FLOAT_TOL), 'bad vectors from PCA'
  ...
  def testTransform(self):
    ...
    refPs = [
      numpy.array([-1.20362098, -1.79265006, 0.08776266]),
  ...
  def testTransform2(self):
    ...
    self.d = numpy.array([[1, 2.068703704, 2.040555556, 2.068703704, 2.141782407, 7.46],
                          ...
                          [22, 2.142199074, 1.276206538, 1.178894558, 2.178819444, 9.52]], 'd')
    self.d = self.d[:, 1:-2]
    _, eVects = Stats.PrincipalComponents(self.d)
    pts = Stats.TransformPoints(eVects, self.d)
    ...
    for i in range(len(pts)):
      for j in range(len(pts)):
        ...
        assert feq(numpy.dot(vi, vj), numpy.dot(
          pvi, pvj)), 'bad dot: %4.4f %4.4f' % (numpy.dot(vi, vj), numpy.dot(pvi, pvj))
