  ...
  while i < len(final) and counts[i] != 0:
    if nModels > 1:
      ...
      outF.write('%d %f %f %d %f\n' %
                 (i + 1, final[i][0] / counts[i], final[i][1] / counts[i], counts[i], confInterval))
    else:
      outF.write('%d %f %f %d\n' %
                 (i + 1, final[i][0] / counts[i], final[i][1] / counts[i], counts[i]))
    i += 1
  outF.close()
  ...
  if details.plotFile:
    ...
    while i < nPts and counts[i] != 0:
      if nModels > 1:
        ...
        print('%d\t%f\t%f\t%d\t%s' %
              (i + 1, final[i][0] / counts[i], final[i][1] / counts[i], counts[i], str(selPts[i])))
      i += 1
