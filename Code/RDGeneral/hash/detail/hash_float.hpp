...
#if defined(__OpenBSD__)
template <>
struct limits<long double> : std::numeric_limits<long double> {
...
inline std::hash_result_t float_hash_impl(T v) {
  ...
  if (v < 0) {
    v = -v;
    exp += limits<T>::max_exponent - limits<T>::min_exponent;
  }
  ...
  std::hash_result_t const length =
      (limits<T>::digits * boost::static_log2<limits<T>::radix>::value - 1) /
