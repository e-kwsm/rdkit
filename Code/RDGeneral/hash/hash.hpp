...
inline std::hash_result_t hash_value_signed(T val) {
  const int hash_result_t_bits =
      std::numeric_limits<std::hash_result_t>::digits;
  // ceiling(std::numeric_limits<T>::digits / hash_result_t_bits) - 1
  const int length = (std::numeric_limits<T>::digits - 1) / hash_result_t_bits;

  std::hash_result_t seed = 0;
  T positive = val < 0 ? -1 - val : val;

  // Hopefully, this loop can be unrolled.
  for (unsigned int i = length * hash_result_t_bits; i > 0;
       i -= hash_result_t_bits) {
...
template <class T>
struct hash : public gboost::hash_detail::hash_impl<
                  boost::is_pointer<T>::value>::BOOST_NESTED_TEMPLATE inner<T> {
};
#endif
}  // namespace gboost
