...
static const float ORIGINAL_FONT_SIZE = 35.0;

// ****************************************************************************
QColor &colour_for_atomic_num(int atnum) {
  static QColor colours[10] = {
      QColor("Black"),  QColor("Blue"),   QColor("Red"),   QColor("LightGreen"),
      QColor("Orange"), QColor("Yellow"), QColor("Green"), QColor("Brown"),
...
// ****************************************************************************
void set_scale(int width, int height, int min_x, int min_y, int qp_width,
               int qp_height, QPainter &qp, qreal &gscale) {
  ...
  qp.translate(-min_x + 0.5 * (qp_width_in_scale - width),
               -min_y + 0.5 * (qp_height_in_scale - height));
}
...
void draw_atom(vector<int>::iterator &pos, bool bow, QPainter &qp) {
  ...
  // blank out behind the letters, so bonds don't go through where the label is.
  // I think circles work better than rectangles for this, but it means going
  // through it all twice, as the circles overlap so would obscure the letter
  // that came afterwards This assumes the bonds are drawn before the atoms,
  // which is how it is set up in MolDrawing.h.
  ...
  // do things 1 letter at a time. We needed to stuff the characters into label
  ...
  for (int i = 0; i < slen; ++i) {
    ...
    // subscript for numbers, superscript for charge. Stretch the ellipse out so
    // the bond is obscured over the whole of its extent.
    ...
    qp.drawEllipse(
        QRectF(-2.0, -2.0 + letter_top, br.width() + 2.0, letter_height + 2.0));
  ...
  for (int i = 0; i < slen; ++i) {
    ...
    qp.drawText(QRectF(0.0, 0.0, br.width(), br.height()), Qt::AlignHCenter,
                at_lab, &br);
    qp.translate(br.width(), 0.0);
...
// ****************************************************************************
void line_extremes(vector<int>::const_iterator &pos, int &min_x, int &max_x,
                   int &min_y, int &max_y) {
...
// ****************************************************************************
void atom_extremes(vector<int>::const_iterator &pos, QPainter &qp, int &min_x,
  ...
  pos++;  // move past orient flag, which is not being used, and indeed is not
          // understood!
  ...
  // subscripts and superscripts may occur in the atom label. Just allow for
  // that willy-nilly.
...
// ****************************************************************************
void setup_scale(vector<int> &drawing, QPainter &qp, int qp_width,
                 int qp_height) {
  // calculate the width of the drawing. The values for width and height as
  // supplied in drawing by MolDrawing.h don't allow for the width of atom
  // labels so if there are non-carbon atoms at the extremes of the picture,
  // these will be drawn off the page.
...
// ****************************************************************************
void MolToQPainter(vector<int> &drawing, int qp_width, int qp_height, bool bow,
                   QPainter &qp) {
...
void smiles_to_qpainter(const string &smiles, const QString &label, int width,
                        int height, bool bow, QPainter &qp) {
  ...
  // get the height that will be needed for the label, and allow the picture to
  // accommodate it
  QRect br;
  if (!label.isEmpty()) {
