...
class TestCase(unittest.TestCase):
  ...
  def test1aPoint3D(self):
    ...
    self.assertAlmostEqual(pt.z, 0.0, delta=1e-4)

    pt = geom.Point3D(3., 4., 5.)
    self.assertAlmostEqual(pt.x, 3.0, delta=1e-4)
    self.assertAlmostEqual(pt.y, 4.0, delta=1e-4)
    self.assertAlmostEqual(pt.z, 5.0, delta=1e-4)
    self.assertAlmostEqual(pt[0], 3.0, delta=1e-4)
    self.assertAlmostEqual(pt[1], 4.0, delta=1e-4)
    self.assertAlmostEqual(pt[2], 5.0, delta=1e-4)
    self.assertAlmostEqual(pt[-3], 3.0, delta=1e-4)
    self.assertAlmostEqual(pt[-2], 4.0, delta=1e-4)
    self.assertAlmostEqual(pt[-1], 5.0, delta=1e-4)
    lst = list(pt)
    self.assertAlmostEqual(lst[0], 3.0, delta=1e-4)
    self.assertAlmostEqual(lst[1], 4.0, delta=1e-4)
    self.assertAlmostEqual(lst[2], 5.0, delta=1e-4)
    ...
    self.assertAlmostEqual(pt3.z, 6.0, delta=1e-4)

    pt += pt2
    self.assertAlmostEqual(pt.x, 4.0, delta=1e-4)
    self.assertAlmostEqual(pt.y, 5.0, delta=1e-4)
    self.assertAlmostEqual(pt.z, 6.0, delta=1e-4)

    pt3 = pt - pt2
    self.assertAlmostEqual(pt3.x, 3.0, delta=1e-4)
    self.assertAlmostEqual(pt3.y, 4.0, delta=1e-4)
    self.assertAlmostEqual(pt3.z, 5.0, delta=1e-4)

    pt -= pt2
    self.assertAlmostEqual(pt.x, 3.0, delta=1e-4)
    self.assertAlmost(pt.y, 4.0, delta=1e-4)
    self.assertAlmost(pt.z, 5.0, delta=1e-4)

    pt *= 2.0
    self.assertAlmost(pt.x, 6.0, delta=1e-4)
    self.assertAlmost(pt.y, 8.0, delta=1e-4)
    self.assertAlmost(pt.z, 10.0, delta=1e-4)

    pt /= 2
    self.assertAlmost(pt.x, 3.0, delta=1e-4)
    self.assertAlmost(pt.y, 4.0, delta=1e-4)
    self.assertAlmost(pt.z, 5.0, delta=1e-4)
    self.assertAlmost(pt.Length(), 7.0711, delta=1e-4)
    self.assertAlmost(pt.LengthSq(), 50.0, delta=1e-4)
    pt.Normalize()
    self.assertAlmost(pt.Length(), 1.0, delta=1e-4)

    pt1 = geom.Point3D(1.0, 0.0, 0.0)
    pt2 = geom.Point3D(2.0 * math.cos(math.pi / 6), 2.0 * math.sin(math.pi / 6), 0.0)
    ang = pt1.AngleTo(pt2)
    self.assertAlmost(ang, math.pi / 6, delta=1e-4)

    prod = pt1.DotProduct(pt2)
    self.assertAlmost(prod, 2.0 * math.cos(math.pi / 6), delta=1e-4)

    pt3 = pt1.CrossProduct(pt2)
    self.assertAlmost(pt3.x, 0.0, delta=1e-4)
    self.assertAlmost(pt3.y, 0.0, delta=1e-4)
    self.assertAlmost(pt3.z, 1.0, delta=1e-4)

  def test1bPoint2D(self):
    pt = geom.Point2D()
    self.assertAlmost(pt.x, 0.0, delta=1e-4)
    self.assertAlmost(pt.y, 0.0, delta=1e-4)
    ...
    self.assertAlmost(pt.x, 3.0, delta=1e-4)
    self.assertAlmost(pt.y, 4.0, delta=1e-4)
    self.assertAlmost(pt[0], 3.0, delta=1e-4)
    self.assertAlmost(pt[1], 4.0, delta=1e-4)
    self.assertAlmost(pt[-2], 3.0, delta=1e-4)
    self.assertAlmost(pt[-1], 4.0, delta=1e-4)
    lst = list(pt)
    self.assertAlmost(lst[0], 3.0, delta=1e-4)
    self.assertAlmost(lst[1], 4.0, delta=1e-4)

    pt2 = geom.Point2D(1., 1.)

    pt3 = pt + pt2
    self.assertAlmost(pt3.x, 4.0, delta=1e-4)
    self.assertAlmost(pt3.y, 5.0, delta=1e-4)

    pt += pt2
    self.assertAlmost(pt.x, 4.0, delta=1e-4)
    self.assertAlmost(pt.y, 5.0, delta=1e-4)

    pt3 = pt - pt2
    self.assertAlmost(pt3.x, 3.0, delta=1e-4)
    self.assertAlmost(pt3.y, 4.0, delta=1e-4)
    ...
    self.assertAlmost(pt.x, 3.0, delta=1e-4)
    self.assertAlmost(pt.y, 4.0, delta=1e-4)
    self.assertAlmost(pt.Length(), 5.0, delta=1e-4)
    self.assertAlmost(pt.LengthSq(), 25.0, delta=1e-4)
    pt.Normalize()
    self.assertAlmost(pt.Length(), 1.0, delta=1e-4)
  ...
  def test1cPointND(self):
    ...
    pt = geom.PointND(4)
    for i in range(dim):
      self.assertAlmost(pt[i], 0.0, delta=1e-4)

    pt[0] = 3
    pt[3] = 4
    self.assertAlmost(pt[0], 3.0, delta=1e-4)
    self.assertAlmost(pt[3], 4.0, delta=1e-4)
    self.assertAlmost(pt[-4], 3.0, delta=1e-4)
    self.assertAlmost(pt[-1], 4.0, delta=1e-4)
    lst = list(pt)
    self.assertAlmost(lst[0], 3.0, delta=1e-4)
    self.assertAlmost(lst[3], 4.0, delta=1e-4)
    ...
    pt3 = pt + pt2
    self.assertAlmost(pt3[0], 4.0, delta=1e-4)
    self.assertAlmost(pt3[2], 1.0, delta=1e-4)
    self.assertAlmost(pt3[3], 4.0, delta=1e-4)

    pt += pt2
    self.assertAlmost(pt[0], 4.0, delta=1e-4)
    self.assertAlmost(pt[2], 1.0, delta=1e-4)
    self.assertAlmost(pt[3], 4.0, delta=1e-4)

    pt3 = pt - pt2
    self.assertAlmost(pt3[0], 3.0, delta=1e-4)
    self.assertAlmost(pt3[2], 0.0, delta=1e-4)
    self.assertAlmost(pt3[3], 4.0, delta=1e-4)

    pt -= pt2
    self.assertAlmost(pt[0], 3.0, delta=1e-4)
    self.assertAlmost(pt[2], 0.0, delta=1e-4)
    self.assertAlmost(pt[3], 4.0, delta=1e-4)

    pt *= 2.0
    self.assertAlmost(pt[0], 6.0, delta=1e-4)
    self.assertAlmost(pt[1], 0.0, delta=1e-4)
    self.assertAlmost(pt[2], 0.0, delta=1e-4)
    self.assertAlmost(pt[3], 8.0, delta=1e-4)
    ...
    for i in range(len(pt)):
      self.assertAlmost(pt2[i], pt[i], delta=1e-4)
  ...
  def testPointPickles(self):
    ...
    self.assertAlmost(pt.x, pt2.x, delta=1e-6)
    self.assertAlmost(pt.y, pt2.y, delta=1e-6)
    self.assertAlmost(pt.z, pt2.z, delta=1e-6)
  ...
  def test5GridOps(self):
    ...
    grd4.SetSphereOccupancy(geom.Point3D(-2.0, 2.0, 0.0), 1.0, 0.25)
    grd4.SetSphereOccupancy(geom.Point3D(2.0, -2.0, 0.0), 1.0, 0.25)
    self.assertAlmost(geom.TanimotoDistance(grd4, grd), .3333, delta=1e-4)
    self.assertAlmost(geom.TanimotoDistance(grd4, grd2), .75, delta=1e-4)

    grd4 &= grd2
    self.assertAlmost(geom.TanimotoDistance(grd4, grd), 1.0, delta=1e-4)
    self.assertAlmost(geom.TanimotoDistance(grd4, grd2), .5, delta=1e-4)
