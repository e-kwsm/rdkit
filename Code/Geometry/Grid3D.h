...
class RDKIT_RDGEOMETRYLIB_EXPORT Grid3D {
 public:
  ...
  [[nodiscard]] virtual ValueType1 getVal(const Point3D &point) const = 0;
  ...
  [[nodiscard]] virtual ValueType2 getVal(unsigned int pointId) const = 0;
  virtual void setVal(unsigned int pointId, ValueType2 val) = 0;
  [[nodiscard]] virtual unsigned int getSize() const = 0;
  [[nodiscard]] virtual const VectorType *getOccupancyVect() const = 0;
};
}  // namespace RDGeom
