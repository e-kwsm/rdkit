...
struct EBV_wrapper {
  static void wrap() {
    python::class_<ExplicitBitVect, boost::shared_ptr<ExplicitBitVect>>(
        ...
        .def("__getitem__",
             static_cast<int (*)(const EBV &, int)>(get_VectItem),
             python::args("self", "which"))
        .def("__setitem__", static_cast<int (*)(EBV &, int, int)>(set_VectItem),
             python::args("self", "which", "val"))
        .def("GetOnBits", static_cast<IntVect (*)(const EBV &)>(GetOnBits),
             python::args("self"),
             "Returns a tuple containing IDs of the on bits.\n")
        .def("ToBinary",
             static_cast<python::object (*)(const EBV &)>(BVToBinary),
             python::args("self"),
             "Returns an internal binary representation of the vector.\n")
        .def("FromBase64",
             static_cast<void (*)(EBV &, const std::string &)>(InitFromBase64),
             python::args("self", "inD"),
             "Initializes the vector from a base64 encoded binary string.\n")
        .def("ToBase64", static_cast<std::string (*)(EBV &)>(ToBase64),
             python::args("self"),
             "Converts the vector to a base64 string (the base64 encoded "
             "version of the results of ToString()).\n")
        .def("ToList",
             static_cast<python::list (*)(const EBV &)>(ExplicitToList),
             python::args("self"),
