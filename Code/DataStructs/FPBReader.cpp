...
RDKIT_DATASTRUCTS_EXPORT boost::dynamic_bitset<> *bytesToBitset(
  ...
  if (!(nBytes % sizeof(boost::dynamic_bitset<>::block_type))) {
    ...
    return reinterpret_cast<boost::dynamic_bitset<> *>(
        new std::dynamic_bitset<std::uint8_t>(fpData, fpData + nBytes));
  }
}
...
std::shared_array<std::uint8_t> FPBReader::getBytes(unsigned int idx) const {
  PRECONDITION(df_init, "not initialized");

  return std::shared_array<std::uint8_t>(detail::copyBytes(dp_impl, idx));
};
