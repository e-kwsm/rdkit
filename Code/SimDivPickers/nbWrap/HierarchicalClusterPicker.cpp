...
RDKit::INT_VECT HierarchicalPicks(
    HierarchicalClusterPicker *picker,
    nb::ndarray<nb::numpy, const double, nb::ndim<1>, nb::c_contig> distMat,
    int poolSize, int pickSize) {
  if (pickSize >= poolSize) {
    throw nb::value_error("pickSize must be less than poolSize");
  }

  std::vector<double> dMatCopy(distMat.data(),
                               distMat.data() + distMat.shape(0));
  return picker->pick(dMatCopy.data(), poolSize, pickSize);
}

// REVIEW: the poolSize can be pulled from the numeric array
RDKit::VECT_INT_VECT HierarchicalClusters(
    HierarchicalClusterPicker *picker,
    nb::ndarray<nb::numpy, const double, nb::ndim<1>, nb::c_contig> distMat,
    int poolSize, int pickSize) {
  std::vector<double> dMatCopy(distMat.data(),
...
void wrap_HierarchCP(nb::module_ &m) {
  nb::class_<RDPickers::HierarchicalClusterPicker>(
      ...
      .def(nb::init<RDPickers::HierarchicalClusterPicker::ClusterMethod>(),
           "clusterMethod"_a)
      .def(
          "Pick", RDPickers::HierarchicalPicks, "distMat"_a, "poolSize"_a,
          "pickSize"_a,
...
ARGUMENTS:
  ...
  - pickSize: number of items to pick from the pool
      ...
      .def(
          "Cluster", RDPickers::HierarchicalClusters, "distMat"_a, "poolSize"_a,
          "pickSize"_a,
