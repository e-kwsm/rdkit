...
// https://nanobind.readthedocs.io/en/latest/classes.html#instance-pooling
  ...
  NOTE:
     ...
     .def("SetName", &ROMol::setName, "name"_a, R"DOC(Sets the molecule name stored as the _Name property.
     ARGUMENTS:
          - name: the name to set (a string).
)DOC")
        .def("GetName", &ROMol::getName, R"DOC(Returns the molecule name stored as the _Name property.
     NOTE:
        ...
        .def(
            ...
            [](const ROMol &self, const QueryAtom *qa) {
              ...
              if (qa && hasRecursiveQuery(*qa)) {
                ...
                detail::MatchSubqueries(self, qa->getQuery(), params,
                                        subqueryMap, locker.locked);
              }
              return QueryAtomIterSeq(self, qa, ownsQa);
            },
